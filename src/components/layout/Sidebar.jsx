...
const Sidebar = ({ 
  ...
  return (
    <>
      {/* Этот div затемняет фон на мобильных устройствах, когда меню открыто */}
      {isOpen && <div onClick={onClose} className="fixed inset-0 bg-black bg-opacity-50 z-40 lg:hidden"></div>}
      
      {/* Основной контейнер сайдбара */}
      <aside className={`fixed top-0 left-0 h-full w-80 bg-white border-r border-stone-200 flex flex-col p-4 transform transition-transform duration-300 ease-in-out z-50 ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center">
            <div className="p-2 bg-teal-100 rounded-lg"><MessageSquare className="text-teal-600" /></div>
            <h1 className="text-2xl font-bold ml-3 text-stone-800">Рефлексия</h1>
