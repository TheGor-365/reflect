...
const Sessions = ({ sessions, profile, userId, currentSessionId, setCurrentSessionId }) => {
  ...
  const handleSendMessage = async (e) => {
    ...
    const chatHistory = activeSession ? [...activeSession.chatHistory, userMessage] : [userMessage];
    ...
    try {
      ...
      const sessionPayload = { title: aiResponse.sessionTitle || "Новый сеанс", chatHistory: newChatHistory, score: score, recommendations: aiResponse.recommendations || [], exercises: aiResponse.exercises || [] };
    ...
    <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
      <div className="flex flex-col bg-transparent rounded-xl h-full">
        <div className="p-1 md:p-2 flex-1 overflow-y-auto">
          {isNewSessionView && <ChatMessage message={{role: 'assistant', content: `Здравствуйте, ${profile?.name || 'пользователь'}! Что вас беспокоит сегодня?`, id: 'initial-message'}} onPlayAudio={handlePlayAudio} isSpeaking={isSpeaking} currentlySpeakingId={currentlySpeakingId} />}
          {activeSession?.chatHistory.map((msg, index) => <ChatMessage key={msg.id || index} message={msg} onPlayAudio={handlePlayAudio} isSpeaking={isSpeaking} currentlySpeakingId={currentlySpeakingId} />)}
          {isLoading && <ChatMessage message={{role: 'assistant', content: 'Ассистент печатает...'}} onPlayAudio={() => {}} />}
          <div ref={chatEndRef} />
        </div>
